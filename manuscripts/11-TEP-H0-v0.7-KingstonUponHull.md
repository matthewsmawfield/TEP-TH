...
First published: 11 January 2026 · Last updated: 17 June 2026  
...
(TEP correction with free $\kappa_{\rm Cep}$ vs. null) in the
host-contrast likelihood, which projects out the shared calibration mode
and tests only the environmental structure, yields $\Delta$BIC = 88 and a
Bayes factor of $\approx 10^{19}$, strongly favoring the TEP predictor
over the null, quantifying the evidence for a TEP environmental predictor with cross-domain structural consistency despite $N=29$.
...
($\langle S \rangle = 0.946$). On this interpretation, the M31 HST
"Inner Fainter" signal has the predicted sign and magnitude for
density-dependent shear attenuation, but the current heterogeneous
catalog does not yet discriminate a continuous suppression profile
from a step-like photometric-regime transition.
...
coefficient. A microscopic transfer calculation remains a separate task.
...
environmental structure—yields ΔBIC = 88 and a Bayes factor of
...
(ΔBIC ≈ −3) because the shared calibration uncertainty dominates the
...
A diagonal H0-uncertainty check gives ΔBIC = 94 as an independent
robustness verification.

**Host-contrast projection.** The host-contrast likelihood
projects out the shared calibration mode by evaluating the model on
contrasts relative to the calibration-weighted mean. With projection
matrix

\begin{equation}
\mathbf{P} = \mathbf{I} - \mathbf{1}(\mathbf{1}^{\mathsf{T}}\mathbf{C}^{-1}\mathbf{1})^{-1}\mathbf{1}^{\mathsf{T}}\mathbf{C}^{-1},
\label{eq:projection_matrix}
\end{equation}

the likelihood is evaluated on $\mathbf{P}\mathbf{H}_0$, or equivalently
on common-mode-subtracted host contrasts. This removes the dominant
calibration uncertainty and isolates the environmental-structure
evidence. The sign convention is
$\Delta{\rm BIC} \equiv {\rm BIC}_{\rm null} - {\rm BIC}_{\rm TEP}$,
so that positive values favor the TEP model.

| Likelihood | $\Delta$BIC | Role |
| --- | --- | --- |
...
![Scatter plot showing positive correlation between H0 and host galaxy velocity dispersion squared (Spearman rho=0.511, p=0.0046), with high-sigma hosts yielding systematically higher H0 values and NGC 4639 annotated as an outlier](public/figures/figure_01_h0_vs_sigma.png)
...
spectroscopy and 13 from kinematic proxies (10 HI linewidth, 3 rotation
...
![Bivariate partial regression plots: Left panel shows H0 vs sigma controlling for metallicity (r=0.43, p=0.022); Right panel shows H0 vs metallicity controlling for sigma (r=0.25, p=0.20, not significant)](public/figures/figure_02_bivariate_h0_sigma_metallicity.png)
...
![Side-by-side comparison: Left panel shows original SH0ES data with clear H0-sigma dependence; Right panel shows TEP-corrected data with environmental trend eliminated and mean H0=68.17 km/s/Mpc aligned with Planck](public/figures/figure_03_tep_correction_comparison.png)
...
mag, $\sigma^2/c^2$ scaling). The corrected panel shows the fitted
correction removes the host-$\sigma$ trend by construction; independent
robustness is assessed by jackknife, host-contrast likelihood,
train/test, anchor, and systematics tests. The unified mean
...
![Line plot showing unified TEP-corrected H0 remains consistent with Planck (red dashed line at 67.4) across reference sigma values from 55 to 95 km/s, demonstrating robustness to calibration parameter choice](public/figures/figure_04_sensitivity_h0_vs_sigmaref.png)
...
ref}$. The mean unified $H_0$ (dark curve) and fixed-$\kappa_{\rm
Cep}$ curve (blue) are each surrounded by a grey $1\sigma$ uncertainty
envelope (shaded band) representing the standard error on the mean
across the host sample. The unified $H_0$ remains statistically
consistent with the Planck baseline ($\le 1.5\sigma$) for a wide range
of physically motivated reference values ($\sigma_{\rm ref} \in [55,
95]$ km/s).
...
![Joint environmental-screening model fit to 29 SN Ia hosts and 3 geometric anchors](public/figures/figure_05_joint_screening_model.png)
...
host-contrast likelihood, which yields $\Delta$BIC = 88 and a
...
![Jackknife influence analysis plot showing how the Pearson correlation between H0 and host velocity dispersion changes when each host is removed](public/figures/figure_06_jackknife_influence.png)

Figure 6: Jackknife influence analysis. No single host galaxy dominates
the correlation; removing any individual host leaves the environmental
trend intact ($r > 0.429$ in all subsamples), confirming the signal is a
global feature of the SH0ES sample rather than an artifact of a few
...
![Scatter plot of differential distance modulus (mu_TRGB minus mu_Cepheid) versus host velocity dispersion for 13 hosts, showing positive correlation (r=0.51, p=0.073) consistent with TEP prediction that Cepheid distances become underestimated in high-potential hosts](public/figures/figure_07_trgb_cepheid_residual.png)

Figure 7: Differential distance modulus ($\mu_{\rm TRGB} - \mu_{\rm
Cepheid}$) versus host velocity dispersion for 13 hosts. A positive
correlation (Pearson $r=0.51$, $p=0.073$; Spearman $\rho=0.57$,
$p=0.041$; slope $0.15\pm0.07$ mag/dex) is observed, with sign
compatible with the TEP prediction that Cepheid distances become
systematically underestimated in high-potential hosts relative to TRGB.
The current $N=13$ TRGB overlap sample shows the predicted positive
sign, but the result remains suggestive at current precision
($p=0.073$); the non-parametric trend is more robust ($p=0.041$), and
the presence of this differential signature provides sign-compatible
...
![Multi-panel synthesis showing Inner Fainter offsets in both ground-based and HST M31 data consistent with TEP shear suppression; LMC control shows no large offset indicating no large pipeline artifacts](public/figures/figure_08_robustness_synthesis_plot.png)
...
![Three-panel figure showing M31 P-L residuals versus radius (color-coded by HST/PHAT vs ground-based source), residuals versus suppression factor S, and radially binned residuals with Models B and C overlaid](public/figures/figure_09_m31_radial_suppression.png)

Figure 9: M31 radial shear-suppression model comparison. Panel (a): P-L
residuals versus galactocentric radius, color-coded by photometric source
(HST/PHAT in blue, ground-based in orange), with the continuous Model C
prediction overlaid. Panel (b): Residuals versus shear suppression factor
...
binned predictions of both Model C (continuous) and Model B (step-function,
AIC-preferred). While the step model is preferred by AIC due to photometric
regime transitions, both models agree on the sign and approximate magnitude
of the environmental offset.
...
![Four-panel figure showing continuous shear-suppression framework: sigma vs density colored by S, suppression curve with host rug marks, response coefficient vs sigma, and correction magnitude vs S](public/figures/figure_10_shear_suppression.png)

Figure 10: The TEP continuous shear-suppression framework applied to
...
response coefficient $\kappa_{\rm Cep} \approx 1.05\times10^6$ mag. Panel (d): Absolute correction magnitude $|\Delta\mu| = \kappa_{\rm Cep} S \, (\sigma^2 - \sigma_{\rm ref}^2)/c^2$ versus $S$,
...
![Sensitivity analysis showing H0-sigma correlation is stable across both raw literature velocity dispersions and aperture-corrected values, with consistent results under full aperture/size sensitivity envelope](public/figures/figure_11_aperture_sensitivity.png)

Figure 11: Sensitivity of the correlation to aperture corrections.
...
Across the full parameter envelope
($\beta \in [0.00, 0.08]$ and $R/R_{\rm eff} \in [0.70, 1.30]$),
the Pearson coefficient varies only between $r = 0.457$ and
$r = 0.466$, confirming the kinematic signal is invariant to
aperture correction models.
...
![Forest plot of standardized regression coefficients showing Velocity Dispersion (Potential Proxy) remains the dominant predictor of H0 across Baseline, AgeControl, DustControl, Full, and FlowEnvironment model specifications](public/figures/figure_12_multivariate_robustness.png)
...
comparison (TEP with free $\kappa_{\rm Cep}$ vs. null) in the
host-contrast likelihood yields $\Delta$BIC = 88 and a Bayes
...
in the host-contrast likelihood yields $\Delta$BIC = 88 and a Bayes factor
...
- **HI linewidth proxy:** 10 hosts use HI 21-cm linewidth measurements calibrated via $\sigma = 0.467 \times V_{\rm max} + 42.9$ km/s (HyperLEDA calibrated_vmax mode). This introduces additional scatter but preserves the kinematic nature of the observable.
...
The correlation coefficient strengthens when restricting to stellar-absorption-only hosts ($N=16$, Pearson $r = 0.554$, $p = 0.026$). Critically, the 13 kinematic-proxy hosts (10 HI + 3 rotation) do not cluster anomalously—they span the full $\sigma$–$H_0$ distribution and follow the same physical trend as stellar hosts (see Section 3.2). Application of the TEP correction to the stellar-only subsample yields a unified $H_0 = 66.15 \pm 1.59$ km/s/Mpc, consistent with the full-sample result.
...
### C.15 Numerical MESA/RSP Validation of the Scalar-Boundary Transport Law
...
We treat the period produced by the MESA/RSP nonlinear pulsation
test-suite model (or, when MESA is not installed, a representative
literature placeholder) as the matter-frame pulsation period
...
correction law to machine precision.