...
- **Matter-Frame Curvature**: Prove that for $A_{\rm clock}(\eta)=C\eta^{-p}$ with $0 < p<1$, all curvature invariants generated from the Ricci sector vanish; since the Weyl tensor vanishes in the conformally flat branch, all polynomial curvature invariants vanish at the temporal conformal boundary

- **Geodesic Completeness**: Demonstrate that for $0 < p\le\tfrac12$ the temporal horizon is simultaneously curvature-empty, timelike-complete, and null-complete
...
The extra gradient term $12A_{\rm clock}^{-4}(\nabla A_{\rm clock})^{2}$, omitted in the schematic expression used in earlier drafts, is essential. In the TEP static background $g_{\mu\nu}$ has $R=0$ and $\nabla A_{\rm clock}$ is spatially homogeneous, so the surviving terms are controlled by the background profile $A_{\rm clock}(\eta)$. For the temporal-horizon profile $A_{\rm clock}(\eta)=C\eta^{-p}$ with $0 < p<1$, the dominant term is $\tilde{R}=6A_{\rm clock}''/A_{\rm clock}^{3}=6p(p+1)C^{-2}\eta^{2p-2}$, which vanishes as $\eta\to\infty$.
...
A key test for physical singularities is geodesic incompleteness. Null and timelike geodesics are integrated backward from the present epoch toward the temporal horizon. For the temporal-horizon profile $A_{\rm clock}(\eta)=C\eta^{-p}$ with $0 < p\le\tfrac12$, both null and timelike geodesics are complete: the null affine parameter diverges and the timelike proper time diverges. For $0 < p<1$ the timelike branch is already complete; null completeness requires the slightly stronger bound $p\le\tfrac12$. The boundary is curvature-empty in this branch.
...
where $\eta$ is the background coordinate time. For the temporal-horizon profile $A_{\rm clock}(\eta)=C\eta^{-p}$ with $0 < p<1$ as $\eta\to\infty$ (early times), the integrand behaves as $\eta^{-2p}$. The integral $\int^{\infty}\eta^{-2p}\,d\eta$ diverges for $2p\le 1$, i.e. for $0 < p\le\tfrac12$. Null geodesics do not terminate in this branch; they extend to $\tilde{\lambda}\to\infty$. For $\tfrac12< p < 1$ the null affine parameter converges, so null completeness requires $p\le\tfrac12$.
...
For the temporal-horizon profile $A_{\rm clock}(\eta)=C\eta^{-p}$ with $0 < p<1$, the integral $\int^{\infty}\eta^{-p}\,d\eta$ *diverges* because $p\le 1$. Timelike proper time to the temporal boundary is therefore *infinite*. This is a major strengthening: timelike geodesics are complete for the entire $0 < p<1$ branch. In TEP, Proposition 1 guarantees that all curvature invariants *vanish* at the temporal horizon for $0 < p<1$, so the infinite proper time reflects that observers can continue evolving indefinitely while the geometry becomes asymptotically curvature-empty.
...
**Theorem (Temporal-horizon geodesic completeness).** Let the conformal clock field take the profile $A_{\rm clock}(\eta)=C\eta^{-p}$ with $0 < p\le\tfrac12$. Then:

- Timelike proper time to the temporal boundary diverges for $0 < p\le 1$, hence in particular for $0 < p\le\tfrac12$.

- Null affine parameter to the temporal boundary diverges for $0 < p\le\tfrac12$.

- All polynomial curvature invariants vanish at the boundary for $0 < p<1$, hence in particular for $0 < p\le\tfrac12$.

- The tensor source term $A_{\rm clock}''/A_{\rm clock}=p(p+1)/\eta^{2}\to 0$ at the boundary.

The cleanest fully complete temporal-horizon branch is therefore $0 < p\le\tfrac12$. In this branch the temporal horizon is simultaneously curvature-empty, timelike-complete, and null-complete.
...
maps $\eta\in[0,\eta_{0}]$ and $r\in[0,\infty)$ to the rectangle $0\le T_{\Lambda}\le\pi/4$, $0\le R<\pi/2$. The bottom edge $T_{\Lambda}=0$ is the Big Bang singularity: a spacelike boundary where $a\to0$ and all curvature invariants diverge. Every causal curve terminates there in finite proper time.
...
- **$\mathscr{T}^{-}$ (temporal past infinity):** the horizontal line $T=0$ ($0\le R<\pi/2$), corresponding to $\eta\to\infty$ at all finite $r$. This is the TEP temporal horizon. It is *not* a curvature singularity; all polynomial invariants vanish there (Proposition 1), and the conformal factor $a_{\rm eff}\to 0$ merely reflects the vanishing of the relative clock rate.

- **Present epoch:** the horizontal line $T=\arctan(1)=\pi/4$ ($0\le R<\pi/2$), corresponding to $\eta=\eta_{0}$ at all finite $r$. This is the observer's present slice.
...
T_{\rm lock} = T_{0}\,(1+z_{t}) \, ,
\end{equation}

where $T_{0}=2.725\,\mathrm{K}$ $=2.35\times10^{-4}$ eV is the present CMB temperature. For the TEP transition redshift $z_{t}\sim 130$,

\begin{equation}
T_{\rm lock} = (2.35\times10^{-4}\,\text{eV})(131) \simeq 0.031\,\text{eV} \approx 0.03\,\text{eV} \, .
\end{equation}

This scale is not chosen by hand; it is fixed by the same transition redshift $z_{t}\sim 100$–$130$ identified in TEP-C0 and TEP-HC. The epoch-screening function
...
| $^{7}$Li/H ($\times 10^{-10}$) | $1.6 \pm 0.3$ | $4.6$ | $4.6$ | $10.0$ |
...
FIRAS constraints are $|\mu| < 9 \times 10^{-5}$ and $|y| < 1.5 \times 10^{-5}$ at 95% CL. The pipeline fits the TEP-predicted spectrum to FIRAS data at multiple redshifts ($z = 100, 500, 1100, 2000$). The fitted $\mu$ and $y$ distortions are consistent with zero within FIRAS constraints: $|\mu| < 10^{-7}$, $|y| < 10^{-8}$. The temporal-horizon thermal mapping preserves a perfect blackbody spectrum without forbidden spectral distortions.
...
For the temporal-horizon profile $A_{\rm clock}(\eta)\sim\eta^{-p}$ with $0 < p<1$, one has
...
both well below the BICEP/Keck 2021 bound $r < 0.036$ (95\% CL). The analytic estimate $r\sim p^2/(4\pi^2\eta_0^3 A_s k_{\rm pivot}^3)\approx 4.5\times 10^{-3}$ is also below the observational bound.

### Robustness of Tensor Suppression

Table 1 reports the maximum tensor-to-scalar ratio $r_{\rm max}$ across the full wavenumber range $k\in[10^{-4},1]$ Mpc$^{-1}$ for variations of the three transition-profile parameters. All values remain well below the BICEP/Keck 2021 bound $r<0.036$.
...
| $p$ | 0.50 | $5.2\times 10^{-3}$ | Even at extreme $p$, $r<0.006\ll 0.036$ |
...
with $A_{\rm clock}=C\eta^{-p}$, $0<p\le\tfrac12$, and kinetic normalization
...
| 2 | Curvature invariants vanish at boundary ($A_{\rm clock}=C\eta^{-p}$, $0 < p<1$) | Proposition 1 (closed-form substitution) | Proven |
| 3 | Null geodesics affine-complete for $0 < p\le\tfrac12$ | Analytic integral + numerical | Proven |
| 3b | Timelike geodesics timelike-complete (infinite proper time) for $0 < p\le 1$ | Analytic integral + numerical | Proven |
...
| --- | --- | --- | --- | --- | --- |
| 5 | BBN abundances ($Y_p$, D/H, $^3$He/H, $N_{\rm eff}$) | $\sim 9\times 10^{-12}$ | $< 4\times 10^{-11}$ | Matches LCDM exactly | Corollary |
...
| 7 | CMB blackbody ($\mu$, $y$ distortions) | Conformal rescaling only | — | $|\mu|<10^{-7}$, $|y|<10^{-8}$ | Corollary |
| 8 | Entropy / arrow of time | — | — | Finite, monotonic $ds/dz < 0$ | Corollary |
...
| 09b | Tensor-to-scalar ratio from native equation: $r(k_{\rm pivot})=9\times 10^{-6}$, $r_{\rm max}=6.26\times 10^{-4}$ | Numerical integration of $\mu_k''+(k^2-A_{\rm clock}''/A_{\rm clock})\mu_k=0$ | Below BICEP/Keck $r<0.036$ |
...
| Curvature invariants | vanish at boundary for $A_{\rm clock}=C\eta^{-p}$, $0 < p<1$ |
| Null geodesics | affine-complete for $0 < p\le\tfrac12$ |
| Timelike geodesics | timelike-complete (infinite proper time) for $0 < p\le 1$ |
...
- For the temporal-horizon profile $A_{\rm clock}(\eta)=C\eta^{-p}$ with $0 < p\le\tfrac12$, all polynomial curvature invariants vanish at the temporal conformal boundary; the boundary is asymptotically curvature-empty, timelike-complete, and null-complete

- Timelike proper time diverges for $0 < p\le 1$; null affine parameter diverges for $0 < p\le\tfrac12$. The boundary is a regular conformal-temporal endpoint, analogous to a smooth spacelike conformal boundary
...
- The thermal screening scale $T_{\rm lock}=T_{0}(1+z_{t})\sim 0.03$ eV is derived from the transition redshift, giving epoch-by-epoch screening ($S_{\rm epoch}\sim 10^{-12}$ at BBN, $\sim 10^{-2}$ at recombination). $T_{\rm lock}$ is not an independent parameter; it is fixed once $z_{t}$ is fixed

- BBN abundances ($Y_p$, D/H, $^3$He/H, $^7$Li/H, $N_{\rm eff}$) are preserved without requiring a physical zero-volume Big Bang

- Recombination and CMB last scattering are validated through $x_e(z)$, $g(z)$, $z_*$, $r_s$, $r_d$, $\theta_s$
...
- Smawfield, M.L. TEP-C0: Temporal Equivalence Principle — Distance-Redshift Reconstruction and Supernova Evidence. *Zenodo* (2026). DOI: 10.5281/zenodo.20370144

- Smawfield, M.L. TEP-HC: Temporal Equivalence Principle — Native hi_class Conformal Implementation, Linear Perturbation Closure, and CMB Acoustic Peak Preservation. *Zenodo* (2026). DOI: 10.5281/zenodo.20682752

- Hawking, S.W. & Penrose, R. The singularities of space-time. *Proc. R. Soc. A* **294**, 529-548 (1966).

- Hawking, S.W. & Penrose, R. The singularities of gravitational collapse and cosmology. *Proc. R. Soc. A* **314**, 529-548 (1970).

- Borde, A., Guth, A.H. & Vilenkin, A. Inflationary spacetimes are not past-complete. *Phys. Rev. Lett.* **90**, 151301 (2003).
...
- Ijjas, A. & Steinhardt, P.J. A brief history of the universe: a new paradigm for the cosmic origin and evolution of matter, space, and time. *Phys. Lett. B* **830**, 137204 (2022).
...
- Riess, A.G., et al. Milky Way Cepheid Standards for Measuring Cosmic Distances and Application to the Hubble Constant. *ApJ* **861**, 126 (2018).
...
- Cyburt, R.H., et al. Big Bang Nucleosynthesis: Status and Projections. *Prog. Part. Nucl. Phys.* **99**, 1-20 (2018).
...
- Zeldovich, Y.B., Sunyaev, R.A. The Interaction of Radiation with Relativistic Electrons in Cosmology. *Astrophys. Space Sci.* **4**, 301 (1969).
...
- Zumalacarregui, M. & Bellini, E. hi_class: Horndeski in the Cosmic Linear Anisotropy Solving System. *Phys. Dark Univ.* **19**, 144-152 (2018).

- Bellini, E., et al. Massive neutrinos in the effective field theory of dark energy. *JCAP* **08**, 053 (2018).

- De Felice, A. & Tsujikawa, S. f(R) Theories. *Living Rev. Rel.* **13**, 3 (2010).

- Wetterich, C. The Universe Equation of State. *Nucl. Phys. B* **302**, 668 (1988).
